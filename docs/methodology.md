...
the SAME two base models (Qwen2.5-7B-Instruct, Mistral-7B-Instruct-v0.3) under nine progressively
...
and time budget. See `src/models/qwen/qlora.py` / `src/models/mistral/qlora.py`.

## Why two base models

Qwen2.5-7B-Instruct is explicitly multilingual-pretrained (Alibaba's own claim covers
23 languages, none of which is Ekegusii); Mistral-7B-Instruct-v0.3 is English-centric
